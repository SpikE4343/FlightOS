...
//-----------------------------------------------------------------------------
	...
	const char* OutputNodesAsString(const ProfileNode *a_root, uint32_t a_count) {
		float fTicksToPc = 100.0f / a_root->data.childTicks.avg;

		char *str = new char[(1 + a_count) * (OUTPUT_WIDTH_SUM + 1) - 1];
		char * s = str;
...
//-----------------------------------------------------------------------------
	...
	const char* OutputZonesAsString(const ProfileZone *a_root, uint32_t a_count) {
		float fTicksToPc = 100.0f / a_root->data.childTicks.avg;
		char* str = new char[(1 + a_count) * (OUTPUT_WIDTH_SUM + 1) - 1];
		char *s = &str[0];
